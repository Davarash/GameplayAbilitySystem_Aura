...
void UAuraAbilitySystemLibrary::SetDebuffDamage(FGameplayEffectContextHandle& EffectContextHandle, float InDebuffDamage)
{
	if (FAuraGameplayEffectContext* AuraEffectContext = static_cast<FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
		AuraEffectContext->SetDebuffDamage(static_cast<float>(InDebuffDamage));
	}
}
...
void UAuraAbilitySystemLibrary::SetIsSuccessfulKnockback(FGameplayEffectContextHandle& EffectContextHandle,
	bool bInSuccessfulKnockback)
{
	if (FAuraGameplayEffectContext* AuraEffectContext = static_cast<FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
		//AuraEffectContext->SetIsSuccessfulKnockback(bInSuccessfulKnockback);
	}
}
*/
	...
	TArray<AActor*>& OutClosestTargets, const FVector& Origin)
{
	...
	while (NumTargetsFound < MaxTargets)
	{
		...
		AActor* ClosestActor;
		for (AActor* PotencialTarget : ActorsToCheck)
		{
			const double Distance = (PotencialTarget->GetActorLocation() - Origin).Length();
			if (Distance < ClosestDistance)
			{
				ClosestDistance = Distance;
				ClosestActor = PotencialTarget;
			}
		}
