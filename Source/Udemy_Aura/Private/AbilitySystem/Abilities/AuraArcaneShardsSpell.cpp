...
#include "AbilitySystem/Abilities/AuraArcaneShardsSpell.h"

FString UAuraArcaneShardsSpell::GetDescription(int32 Level)
{
...
FString UAuraArcaneShardsSpell::GetNextLevelDescription(int32 Level)
{
	const int32 ScaledDamage = Damage.GetValueAtLevel(Level);
	const float ManaCost = FMath::Abs(GetManaCost(Level));
	const float Cooldown = GetCooldown(Level);
