...
#include "AbilitySystem/Abilities/AuraElectrocuteSpell.h"

FString UAuraElectrocuteSpell::GetDescription(int32 Level)
{
...
FString UAuraElectrocuteSpell::GetNextLevelDescription(int32 Level)
{
	const int32 ScaledDamage = Damage.GetValueAtLevel(Level);
	const float ManaCost = FMath::Abs(GetManaCost(Level));
	const float Cooldown = GetCooldown(Level);
