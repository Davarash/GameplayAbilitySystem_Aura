...
void USpellMenuWidgetController::SetButtonsEnabled(const FGameplayTag& AbilityTag)
{
	...
	if (AbilitySpec == NULL)
	{
		...
		ButtonsEnabledDelegate.Broadcast(bSpendPointButtonEnabled, bEquipButtonEnabled);
		return;
	}
	
	const FGameplayTag StatusTag = GetAuraASC()->GetStatusFromSpec(*AbilitySpec);
	const int32 SpellPoints = GetAuraPS()->GetSpellPoints();
	if (StatusTag == FGameplayTag())
	{
		UE_LOG(LogAura, Error, TEXT("Failed to find Status Tag in %hs"), __FUNCTION__)
	}

	/* Equipped and Unlocked*/
	if (SpellPoints > 0 && (StatusTag.MatchesTag(FGameplayTag::RequestGameplayTag(FName("Abilities.Status.Equipped"))) || StatusTag.MatchesTag(FGameplayTag::RequestGameplayTag(FName("Abilities.Status.Unlocked")))))
	{
	...
	/* Eligible */
	if (SpellPoints > 0 && StatusTag.MatchesTag(FGameplayTag::RequestGameplayTag(FName("Abilities.Status.Eligible"))))
	{
		bSpendPointButtonEnabled = true;
		bEquipButtonEnabled = false;
	}
	...
	/* Locked */
	if (SpellPoints > 0 && StatusTag.MatchesTag(FGameplayTag::RequestGameplayTag(FName("Abilities.Status.Locked"))))
	{
		bSpendPointButtonEnabled = false;
		bEquipButtonEnabled = false;
	}
...
void USpellMenuWidgetController::OnAbilityEquipped(const FGameplayTag& AbilityTag, const FGameplayTag& Status, const FGameplayTag& Slot, const FGameplayTag& PreviousSlot)
{
	...
	// Broadcast empty info if PreviousSlot is a valid slot. Only if equipping and already-equipped spell
